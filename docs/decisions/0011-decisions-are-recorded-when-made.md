...
`29c54fc` built the mechanism and `602ab7e` backfilled 0003–0010; the
`decision-records` workflow passed on its first run. The index's tests fail when
a validation rule is removed. The advisory hook fired through the harness on its
first real trigger — this plan marked COMPLETE while this record was still
proposed — which is how this Outcome came to be written.
