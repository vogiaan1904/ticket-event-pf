...
#
# It refuses to kill a node hosting any PVC-backed pod: every EBS volume carries
# node affinity to one availability zone, so a stranded stateful pod sits Pending
# until a replacement node appears in that zone. Stranding redpanda in particular
# fails §6 for reasons that have nothing to do with the saga.
#
# It also refuses to terminate anything until the sold counter shows buyers are
# genuinely mid-purchase: a failure that does not overlap real work proves
# nothing, however green the run looks.
set -euo pipefail
...
# Refuse if the victim hosts any PVC-backed pod, not just postgres: whatever
# stateful pod lives here cannot follow the workload to the survivor. On a
# two-node cluster every node holds one, so this refuses both and says to add a
# third rather than failing an assertion ten minutes later.
...
# DURATION switches purchase.js to constant-vus. Without it every VU buys once
# and exits, draining in ~10s, so the kill would land on an already-idle node.
# The rate stays low deliberately: this gate is about recovery, not throughput.
rm -f "$TCID_FILE"
...
# reliable trigger is the sold counter actually moving.
SOLD_AT_KILL=0
...
KILL_TS=$(date +%s)
# UTC, because that is what `tctl workflow list` prints. Cross-reference it
# against the START/END TIME columns: a saga that resumed is one that started
# before this instant and ended after it.
...
# The column names need double quotes. payment-svc is Prisma, and Prisma maps
# the table (@@map("outbox")) but not the columns: they are created as
# "publishedAt" / "createdAt", so unquoted snake_case does not exist and
# unquoted camelCase is folded to lowercase by Postgres.
