...
Cost is a **first-class design constraint** here, not an afterthought. The hard ceiling is **$20/mo of real spend**, and the projected steady state is ~$17–19/mo — but only because the guardrails below hold. Every one of them exists because the alternative was measured, not guessed.
...
## Where the money actually goes (k3s on EC2, us-east-1)
...
| **k3s box total** | **~$8** | |
| EKS sessions (control plane + spot nodes + ALB, destroyed after) | ~$9–11 | per set of sessions, not standing cost |
...
4. Weekly: glance at AWS Budgets (unblended) and Cost Explorer.
...
**Graviton (`t4g`) is a deferred ~20% lever, not a gate.** It requires arm64 rebuilds of every app *and* infra image, and prior multi-architecture work showed that friction is non-trivial. Documented as a future optimisation.
