...
- **Oversold / reservation issues?** The invariant lives in `inventory-svc/internal/services/reservation.go` — the rule for every counter write is in `inventory-svc/CLAUDE.md`, *Conventions* + the `ReservationExpiryWorker` that auto-releases expired holds.
